...
{
    /// <summary>
    /// Configuration ScriptableObject for random encounter parameters.
    /// All values are clamped both in the inspector (via Range attributes)
    /// and programmatically (via property accessors using Mathf.Clamp).
    /// </summary>
    ...
    public class EncounterConfig : ScriptableObject
    {
        ...
        private float _minTimeBetweenEncounters = 15f;

        [SerializeField, Range(0f, 1f)]
        private float _encounterProbability = 0.15f;

        [SerializeField, Range(1f, 50f)]
        private float _spawnRadius = 10f;

        [SerializeField, Range(2f, 4f)]
        private float _spawnDistanceFromPlayer = 3f;
